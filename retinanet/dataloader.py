...
# imgaug uses removed np.bool/np.int/np.float aliases (NumPy < 1.24 only)
...
last_epoch = 0
new_scale = 0
total_image = [len(glob.glob('/workspace/STAS_VGH/Train_Images/*.jpg'))]
total_epoch = 200
print(min_s, max_s)
...
#         print(annots)
        ...
        if annots.ndim == 2 and annots.shape[0] > 0:
            annots[:, :4] *= scale
...
class Augmenter(object):
    ...
    def __call__(self, sample, flip_x=0.5):
        ...
        # imgaug requires uint8 (0-255); convert from float32 (0-1) if needed
        ...
        # convert back to float32 (0-1) to keep consistent with the rest of the pipeline
